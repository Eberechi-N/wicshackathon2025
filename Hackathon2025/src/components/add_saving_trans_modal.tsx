...
type AddSavingsTransactionModalProps = {
  goal: SavingsGoal
  onClose: () => void
  onSave: (data: Omit<SavingsTransaction, "id">) => void
}

export function AddSavingsTransactionModal({ goal, onClose, onSave }: AddSavingsTransactionModalProps) {
          ...
          >
            <div className="bg-white rounded-lg shadow-lg w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b">
                <Dialog.Title className="text-xl font-semibold">Add Transaction to {goal.name}</Dialog.Title>
              ...
              <form onSubmit={handleSubmit} className="p-4 space-y-4" onClick={(e) => e.stopPropagation()}>
                <div className="space-y-2" onClick={(e) => e.stopPropagation()}>
                  <Label.Root htmlFor="type" className="block text-sm font-medium text-gray-700">
                    Transaction Type
                  </Label.Root>
                  <Select.Root value={formData.type} onValueChange={(value) => handleSelectChange("type", value)}>
                    <Select.Trigger
                      id="type"
